...
{prod} uses https://github.com/containers/gvisor-tap-vsock[gvisor-tap-vsock] to provide user-mode networking on all platforms by default.
...
. {prod} exposes ports 80 and 443 on all interfaces, but the API server (6443) only listens on localhost.
Install [package]`socat` and create a systemd service to forward the API port to the LAN:
+
----
$ sudo dnf install -y socat
----
+
Replace `_REMOTE_IP_` with the IP address of the server (for example, `192.168.1.6`):
+
[subs="+quotes"]
----
...
[Service]
ExecStart=/usr/bin/socat TCP-LISTEN:6443,bind=_REMOTE_IP_,fork,reuseaddr TCP:127.0.0.1:6443
Restart=always
RestartSec=5s
